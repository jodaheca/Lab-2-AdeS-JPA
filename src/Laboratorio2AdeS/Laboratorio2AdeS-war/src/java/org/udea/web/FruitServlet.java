...
public class FruitServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h2>MI TIENDA</h2>");
            Iterator i= fruitsFacade.getFruitsList().iterator();
            while(i.hasNext())
                out.println(i.next()+"<br>");
                out.println("Hay en la BD "+fruitsFacade.count()+" registros<br>");
                try{
                    Fruits f= fruitsFacade.findByName("banano");
                    out.println("El precio del banano es $ " + f.getPrice());
                }catch(Exception e){
                    out.println("No se encontro el dato... " + e.toString());
                }
